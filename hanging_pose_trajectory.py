...
def get_contact_pose_from_trajectory(waypoints : list or np.ndarray, obj_id : int, hook_id : int):
    ...
    contact_object_poses_6d = []
    contact_hook_trajectory_6d = []
    for i in range(len(waypoints) - 1):
        ...
        for index in range(len(positions7d) - 1):
            ...
            obj_transform = get_matrix_from_pos_rot(positions7d[index][:3], positions7d[index][3:])
            contact_pos = (obj_transform @ obj_to_contact_point).reshape(4)[:3]
            ...
            contact_rotvec = R.from_matrix(rotation_mat).as_rotvec()

            # contact relative transform
            contact_transform = get_matrix_from_pos_rot(contact_pos, contact_rotvec)
            
            # contact pose relative to hook
            contact_relative_transform = np.linalg.inv(hook_transform) @ contact_transform
            contact_relative_pos = contact_relative_transform[:3, 3]
            contact_relative_rotvec = R.from_matrix(contact_relative_transform[:3, :3]).as_rotvec()
            contact_relative_pose_6d = list(contact_relative_pos) + list(contact_relative_rotvec)
            contact_hook_trajectory_6d.append(contact_relative_pose_6d)
            ...
            contact_relative_rotvec = R.from_matrix(contact_relative_transform[:3, :3]).as_rotvec()
            contact_relative_pose_6d = list(contact_relative_pos) + list(contact_relative_rotvec)
            contact_object_poses_6d.append(contact_relative_pose_6d)
    
    contact_hook_trajectory_6d = np.asarray(contact_hook_trajectory_6d)
    contact_object_poses_6d = np.asarray(contact_object_poses_6d)

    contact_object_pose_6d = np.mean(contact_object_poses_6d, axis=0)
    return contact_hook_trajectory_6d, contact_object_pose_6d
...
def main(args):
    ...
    input_jsons = [
        ...
        'data/Hook_60-hanging_exp/Hook_60-hanging_exp_wrench_1.json',
        'data/Hook_skew-hanging_exp/Hook_skew-hanging_exp_wrench_1.json',
        'data/Hook_90-hanging_exp/Hook_90-hanging_exp_wrench_1.json',
        'data/Hook_180-hanging_exp/Hook_180-hanging_exp_wrench_1.json',
    ]
    ...
    for input_json in input_jsons:
        ...
        contact_hook_trajectory_6d, contact_object_pose_6d = get_contact_pose_from_trajectory(waypoints=waypoints, obj_id=obj_id, hook_id=hook_id)
        
        # write to file
        if obj_name == 'hanging_exp_wrench_1':
            hook_pos, hook_rot = p.getBasePositionAndOrientation(hook_id)
            hook_pose = list(hook_pos) + list(R.from_quat(hook_rot).as_rotvec())
            trajectory_dict = {
                'trajectory': contact_hook_trajectory_6d.tolist(),
                'hook_pose': hook_pose
            }
        ...
        # write to file
        if obj_name != 'hanging_exp_wrench_1' or hook_name == 'Hook_bar':
            obj_pos, obj_rot = p.getBasePositionAndOrientation(obj_id)
            obj_pose = list(obj_pos) + list(R.from_quat(obj_rot).as_rotvec())
            obj_dict = {
                'contact_pose': contact_object_pose_6d.tolist(),
                'obj_pose': obj_pose
            }
    ...
    # p.resetBasePositionAndOrientation(obj_id, obj_pos, obj_rot)
    # obj_transform = get_matrix_from_pos_rot(obj_pos, obj_rot)
    # contact_transform = get_matrix_from_pos_rot(contact_object_pose_6d[:3], contact_object_pose_6d[3:])
    # contact_transform = obj_transform @ contact_transform
